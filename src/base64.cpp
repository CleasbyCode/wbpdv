...
void binaryToBase64(std::span<const Byte> binary_data, vBytes& output_vec) {
	...
	for (std::size_t i = 0; i < input_size; i += 3) {
		...
		output_vec[out++] = static_cast<Byte>(BASE64_TABLE[(triple >> 18) & 0x3F]);
		output_vec[out++] = static_cast<Byte>(BASE64_TABLE[(triple >> 12) & 0x3F]);
		output_vec[out++] = (i + 1 < input_size)
			? static_cast<Byte>(BASE64_TABLE[(triple >> 6) & 0x3F])
			: static_cast<Byte>('=');
		output_vec[out++] = (i + 2 < input_size)
...
void appendBase64AsBinary(std::span<const Byte> base64_data, vBytes& destination_vec) {
	...
	if (destination_vec.size() > std::numeric_limits<std::size_t>::max() - decoded_upper_bound) {
		throw std::overflow_error("Base64 decode destination size overflow.");
	}
	destination_vec.reserve(destination_vec.size() + decoded_upper_bound);

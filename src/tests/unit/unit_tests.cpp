...
// Like expect_throws, but also requires the message to contain `needle`, so a
// test cannot pass on an unrelated failure that happens to throw.
void expect_throws_message(std::string_view name, std::string_view needle,
                           auto &&fn) {
  try {
    fn();
    ++g_failures;
    ...
    if (what.find(needle) == std::string_view::npos) {
      ++g_failures;
      std::println(std::cerr, "[FAIL] {} (message was: {})", name, what);
      return;
    }
    ++g_passes;
    std::println("[PASS] {}", name);
...
void test_filename_policy() {
  ...
  // embeddedFilenameProblem names the rule that failed. A single catch-all
  // message used to blame a leading '.' or '-' for every rejection, so the
  // distinct reasons matter as much as the rejection itself.
  auto reason = [](const char *problem) {
    return problem == nullptr ? std::string_view("<none>")
                              : std::string_view(problem);
  };
  expect(embeddedFilenameProblem("payload.txt") == nullptr,
         "safe name reports no problem");
  const std::string_view bad_char = reason(embeddedFilenameProblem("a:b.txt"));
  const std::string_view leading_dot = reason(embeddedFilenameProblem(".hidden"));
  const std::string_view leading_dash = reason(embeddedFilenameProblem("-flag"));
  const std::string_view trailing_dot = reason(embeddedFilenameProblem("name."));
  const std::string_view trailing_space =
      reason(embeddedFilenameProblem("name "));
  const std::string_view dotdot = reason(embeddedFilenameProblem(".."));
  expect(bad_char != "<none>" && leading_dot != "<none>" &&
             leading_dash != "<none>" && trailing_dot != "<none>" &&
             trailing_space != "<none>" && dotdot != "<none>",
         "every unsafe name reports a problem");
  expect(bad_char != leading_dot && leading_dash != leading_dot &&
             trailing_dot != leading_dot && trailing_space != trailing_dot &&
             dotdot != leading_dot,
         "distinct rules report distinct reasons");
}

// --- recovered-filename collision naming -----------------------------------

void test_recovery_collision_names() {
  expect(recoveryCollisionCandidate("photo.jpg", 1) == fs::path("photo_1.jpg"),
         "collision name keeps a single extension");
  expect(recoveryCollisionCandidate("archive.tar.gz", 3) ==
             fs::path("archive_3.tar.gz"),
         "collision name keeps a multi-part extension intact");
  expect(recoveryCollisionCandidate("a.b.c.d", 1) == fs::path("a_1.b.c.d"),
         "collision name splits at the first interior dot");
  expect(recoveryCollisionCandidate("README", 2) == fs::path("README_2"),
         "collision name handles a name with no extension");
}
...
void test_output_size_boundaries() {
  expect(MAX_BLUESKY_UPLOAD_SIZE == 2'000'000,
         "Bluesky upload maximum is 2,000,000 bytes");
  expect(checkedStandardOutputSize(MAX_PROGRAM_FILE_SIZE - 100,
                                   /*iccp_needs_padding=*/false,
                                   /*image_size=*/100) ==
...
// Runs fn with `keystrokes` fed through a pipe as stdin, restoring the real
// stdin afterwards on every path. Lets tests drive interactive PIN entry.
template <typename Fn>
auto withPipedStdin(std::string_view keystrokes, Fn &&fn) -> decltype(fn()) {
  int pipe_fds[2] = {-1, -1};
  if (::pipe(pipe_fds) != 0) {
...
std::optional<std::string> decryptWithPinKeystrokes(
  ...
  return withPipedStdin(keystrokes, [&] {
    return decryptDataFile(encrypted_profile, WEBP_EMBED_OFFSETS);
  });
}
...
// placed so findBlueskyWdvMarker accepts it, then varies the TIFF/IFD
...
void test_exif_walker_errors() {
  ...
  // Valid WDV placement but IFD count = 0 -> corrupt EXIF.
  {
    ...
    exif.push_back(0);
    exif.push_back(0);
    // Pad with KDF+WDV so findBlueskyWdvMarker is true.
    while (exif.size() < 20) {
      exif.push_back(0);
    }
...
// --- ICC detector strictness ----------------------------------------------

// The ICC path decrypts at a fixed template offset, so its detector must
// require the WDV marker exactly there. A marker found anywhere else can never
// decrypt, and accepting it cost the user a PIN prompt plus a full Argon2id
// derivation before failing with a misleading corruption message.
void test_icc_detector_requires_exact_marker() {
  vBytes image(WEBP_PROFILE_TEMPLATE.begin(), WEBP_PROFILE_TEMPLATE.end());
  vBytes body = {1, 2, 3, 4, 5, 6, 7};
  zlibDeflate(body);
  (void)encryptDataFile(image, body, "marker.bin", WEBP_EMBED_OFFSETS);

  const std::size_t iccp_data_size = image.size() - ICCP_PAYLOAD_FILE_OFFSET;
  writeLe32At(image, ICCP_RIFF_CHUNK_SIZE_OFFSET,
              static_cast<std::uint32_t>(iccp_data_size));
  writeBe32At(image, ICCP_PROFILE_SIZE_OFFSET,
              static_cast<std::uint32_t>(iccp_data_size));
  if ((iccp_data_size & 1U) != 0) {
    image.push_back(0x00); // RIFF pad byte, as concealStandard emits
  }
  writeLe32At(image, 0x04, static_cast<std::uint32_t>(image.size() - 8));

  // Baseline: untouched, this is recognised as an ICC-embedded wbpdv image and
  // fails only because the piped PIN is empty.
  {
    vBytes intact = image;
    expect_throws_message(
        "intact ICC image is recognised and reaches PIN entry", "Invalid PIN",
        [&] { withPipedStdin("\n", [&] { recoverData(intact); }); });
  }

  // Displace the marker: blank the real one and plant a copy earlier in the
  // profile, leaving every other byte and every size field untouched.
  {
    constexpr std::size_t kPlantedAt = 0x100;
    vBytes displaced = image;
    const std::size_t real_marker =
        WEBP_EMBED_OFFSETS.encrypted_file - WDV_SIGNATURE_SPAN;
    std::memset(displaced.data() + real_marker, 0, WDV_SIGNATURE.size());
    std::memcpy(displaced.data() + kPlantedAt, WDV_SIGNATURE.data(),
                WDV_SIGNATURE.size());
    expect_throws_message(
        "ICC marker at the wrong offset is rejected before PIN entry",
        "Signature check failure",
        [&] { withPipedStdin("\n", [&] { recoverData(displaced); }); });
  }
}

// --- full recovery: payload round-trip + plaintext wipe --------------------

void test_recover_wipes_plaintext() {
  // Build a minimal standard-mode embedded image: profile template with an
  // encrypted payload and no trailing VP8 chunk (recover only walks metadata).
  vBytes image(WEBP_PROFILE_TEMPLATE.begin(), WEBP_PROFILE_TEMPLATE.end());
  vBytes body = {10, 20, 30, 40, 50};
  zlibDeflate(body);
  const std::uint64_t pin =
      encryptDataFile(image, body, "wipe.bin", WEBP_EMBED_OFFSETS);

  const std::uint32_t iccp_data_size =
      static_cast<std::uint32_t>(image.size() - ICCP_PAYLOAD_FILE_OFFSET);
  writeLe32At(image, ICCP_RIFF_CHUNK_SIZE_OFFSET, iccp_data_size);
  writeBe32At(image, ICCP_PROFILE_SIZE_OFFSET, iccp_data_size);
  if ((iccp_data_size & 1U) != 0) {
    image.push_back(0x00); // RIFF pad byte, as concealStandard emits
  }
  writeLe32At(image, 0x04, static_cast<std::uint32_t>(image.size() - 8));
  ...
  for (std::size_t attempt = 0; attempt < 64 && dir.empty(); ++attempt) {
    const fs::path candidate =
        ...
        std::format("wbpdv_recover_unit_{}_{}", ::getpid(),
                    randombytes_uniform(1'000'000));
    std::error_code ec;
    if (fs::create_directory(candidate, ec)) {
      dir = candidate;
    } else if (ec && ec != std::errc::file_exists) {
      throw std::runtime_error("unable to create recover-test directory");
    }
  }
  if (dir.empty()) {
    throw std::runtime_error("unable to allocate recover-test directory");
  }
  ...
  // recover writes the extracted file to the current directory.
  struct CwdGuard {
    fs::path previous;
  ...
  withPipedStdin(std::to_string(pin) + "\n", [&] { recoverData(image); });

  expect(readFile(dir / "wipe.bin", FileTypeCheck::data_file) ==
             vBytes({10, 20, 30, 40, 50}),
         "recoverData round-trips the payload end to end");
  expect(!image.empty() &&
             std::ranges::all_of(image, [](Byte b) { return b == 0; }),
         "recoverData wipes the recovered plaintext buffer");
}
...
void test_atomic_fixed_target_commit() {
  ...
  fs::path collided_stage_path;
  {
    ...
    stage.closeOrThrow();
    expect(!tryCommitPathAtomically(stage.path(), target,
                                    "atomic commit failed: "),
           "atomic commit never replaces existing fixed target");
  ...
  // tryCommitPathAtomically reports EEXIST with a return value so callers can
  // retry the staged file under a different name instead of failing.
  {
    auto try_stage = createUniqueFile(dir, "stage_", ".tmp", 64,
                                      "stage create failed: ",
                                      "stage names exhausted");
    writeAllToFd(try_stage.fd(), new_bytes);
    try_stage.closeOrThrow();
    expect(!tryCommitPathAtomically(try_stage.path(), target,
                                    "try-commit failed: "),
           "try-commit reports an existing target without throwing");
    expect(fs::exists(try_stage.path()),
           "try-commit collision keeps the staged file");
    expect(readFile(target, FileTypeCheck::data_file) == old_bytes,
           "try-commit collision preserves target bytes");
    const fs::path fresh_target = dir / "try-commit-fresh.bin";
    expect(tryCommitPathAtomically(try_stage.path(), fresh_target,
                                   "try-commit failed: "),
           "try-commit moves the stage to a fresh target");
    try_stage.release();
    expect(readFile(fresh_target, FileTypeCheck::data_file) == new_bytes,
           "try-commit preserves staged bytes");
  }
  ...
  expect(tryCommitPathAtomically(stage.path(), target,
                                 "atomic commit failed: "),
         "atomic commit moves stage to a fresh target");
  stage.release();
...
int main() {
  ...
  try {
    ...
    test_icc_detector_requires_exact_marker();
    test_recover_wipes_plaintext();

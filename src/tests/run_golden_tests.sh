...
WORK_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/wbpdv_golden_tests.XXXXXX")"
cleanup() {
    rm -rf -- "$WORK_ROOT"
}
trap cleanup EXIT
...
is_safe_case_id() {
    local case_id="$1"
    [[ ${#case_id} -le 128 && "$case_id" =~ ^[A-Za-z0-9][A-Za-z0-9._-]*$ ]]
}
...
run_case() {
    ...
    if ! is_safe_case_id "$case_id"; then
        echo "[FAIL] manifest: unsafe case_id: $case_id" >&2
        return 1
    fi
    ...
    if ! mkdir "$work"; then
        echo "[FAIL] $case_id: duplicate case_id or unable to create work directory" >&2
        return 1
    fi

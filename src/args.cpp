...
void ProgramArgs::die(const std::string &usage) { throw std::runtime_error(usage); }

std::optional<ProgramArgs> ProgramArgs::parse(int argc, char **argv) {
  const std::string prog = fs::path(argv[0]).filename().string(),
                    usage =
                        std::format("Usage: {} conceal [-b] <cover_image> <secret_file>\n       {} recover <cover_image>\n       {} --info", prog, prog, prog);
  if (argc < 2) die(usage);
  if (std::string_view(argv[1]) == "--info") return displayInfo(), std::nullopt;
  if (std::string_view(argv[1]) == "conceal" && argc >= 4) {
    ...
    if (i + 2 != argc) die(usage);
    return out.image_file_path = argv[i], out.data_file_path = argv[i + 1], out;
  }
  if (std::string_view(argv[1]) == "recover" && argc == 3) return ProgramArgs{Mode::recover, Option::None, argv[2], ""};
  die(usage);
}
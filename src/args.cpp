...
std::optional<ProgramArgs> ProgramArgs::parse(int argc, char** argv) {
  const std::string prog = (argc >= 1 && argv != nullptr && argv[0] != nullptr) ? fs::path(argv[0]).filename().string() : std::string{"wbpdv"};
  ...
  if (command == "recover" && argc == 3) {
    return ProgramArgs{Mode::recover, Option::None, requireArg(argc, argv, 2, usage), ""};
  }

...
Bluesky - Total output image file must not exceed 2,000,000 bytes (use -b option).

	• Cover image + compressed/encrypted data file must fit within 2,000,000 Bytes.
...
──────────────────────────
  ...
  recover - Decrypts, uncompresses and extracts the concealed data file from a WEBP cover image
            ...
            Note: the recovery PIN is numeric and canonicalized — leading zeros are
            ignored, so "00123" and "123" are the same PIN.
...
You must use the Python script "bsky/create_bsky_post.py" to post images to Bluesky.
...
Install its Python dependencies, then provide credentials through environment
variables so the app password is not exposed in the command line shown by `ps`:

$ python3 -m pip install requests beautifulsoup4 pillow
$ export ATP_AUTH_HANDLE=you.bsky.social
$ read -rsp "Bluesky app password: " ATP_AUTH_PASSWORD; export ATP_AUTH_PASSWORD; echo

Here is a basic usage example for the create_bsky_post.py Python script:

Standard image post to your profile/account.

$ python3 bsky/create_bsky_post.py --image your_image.webp \
    --alt-text "alt-text here [optional]" "standard post text here [required]"

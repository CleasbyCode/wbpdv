...
void appendBytes(vBytes &out, std::span<const Byte> bytes) {
  ...
  out.resize(checkedAddSize(offset, bytes.size(),
                            "File Size Error: Output file size overflow."));
  std::memcpy(out.data() + static_cast<std::ptrdiff_t>(offset), bytes.data(),
              bytes.size());
}
...
void writeOutputFile(std::span<const Byte> data, std::uint64_t pin,
                     bool is_bluesky) {
  auto output_file =
      createUniqueFile({}, "wbpdv_", ".webp", 2048,
  ...
  std::println("\nSaved \"file-embedded\" WEBP image: {} ({} bytes).",
               output_file.path.string(), data.size());
...
void concealBluesky(vBytes &image_vec, vBytes &data_vec,
  ...
  writeBe32At(
      ...
      checkedU32(profile_vec.size() - ARTIST_DATA_START,
                 "File Size Error: Artist IFD count exceeds 4GB limit."));

  vBytes output_vec;
  std::size_t output_reserve =
      checkedAddSize(VP8X_END, image_vec.size(),
                     "File Size Error: Output file size overflow.");
  output_reserve =
      checkedAddSize(output_reserve, profile_vec.size() - VP8X_END,
                     "File Size Error: Output file size overflow.");
  output_reserve =
  ...
  appendBytes(output_vec, byteSpan(image_vec));
  appendBytes(output_vec, byteSpan(profile_vec).subspan(VP8X_END));
  appendRiffPadding(output_vec, exif_data_size);
  if (!xmp_chunk.empty()) {
    appendBytes(output_vec, byteSpan(xmp_chunk));
...
void concealStandard(vBytes &image_vec, vBytes &data_vec,
  ...
  writeLe32At(profile_vec, 0x22, iccp_data_size_u32);
  writeBe32At(profile_vec, 0x26, iccp_data_size_u32);
  std::size_t profile_reserve =
      checkedAddSize(profile_vec.size(), image_vec.size(),
                     "File Size Error: Output file size overflow.");
  profile_reserve = checkedAddSize(
      profile_reserve, 1, "File Size Error: Output file size overflow.");
  profile_vec.reserve(profile_reserve);
  appendRiffPadding(profile_vec, iccp_data_size);
  appendBytes(profile_vec, byteSpan(image_vec));
...
void concealData(vBytes &image_vec, Option option,
  ...
  validateConcealSizes(image_vec, data_file_size, is_bluesky);

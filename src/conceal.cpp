...
void writeOutputFile(std::span<const Byte> data, std::size_t pin, bool is_bluesky) {
	...
	if (is_bluesky) {
		std::println("\nBluesky. (Only share this \"file-embedded\" WEBP image on Bluesky).");
	}
...
void concealData(vBytes& image_vec, Option option, const fs::path& data_file_path) {
	...
	if (is_bluesky) {
		...
		// 6b. Encrypt.
		const std::size_t pin = encryptDataFile(profile_vec, data_vec, data_filename, BLUESKY_EMBED_OFFSETS);
	...
	// 6. Encrypt.
	const std::size_t pin = encryptDataFile(profile_vec, data_vec, data_filename, WEBP_EMBED_OFFSETS);

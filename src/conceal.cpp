...
void concealData(vBytes& image_vec, Option option, const fs::path& data_file_path) {
	...
	if (is_bluesky) {
		...
		writeBe32At(profile_vec, BLUESKY_ARTIST_COUNT_OFFSET, checkedU32(profile_vec.size() - ARTIST_DATA_START, "File Size Error: Artist IFD count exceeds 4GB limit."));
		...
		if (output_vec.size() > MAX_BLUESKY_UPLOAD_SIZE) {
			throw std::runtime_error(
				"File Size Error: Output file (" + std::to_string(output_vec.size()) +

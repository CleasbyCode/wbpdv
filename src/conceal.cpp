...
void concealData(vBytes& image_vec, Option option, const fs::path& data_file_path) {
	...
	if (is_bluesky) {
		...
		if (encrypted_size > MAX_EXIF_ENCRYPTED_DATA) {
			...
			auto appendSv = [](vBytes& v, std::string_view s) {
				v.insert(v.end(), reinterpret_cast<const Byte*>(s.data()),
					reinterpret_cast<const Byte*>(s.data() + s.size()));
			};

			vBytes xmp_data;
			xmp_data.reserve(XMP_HEADER.size() + base64_size + XMP_FOOTER.size());
			appendSv(xmp_data, XMP_HEADER);
			binaryToBase64(std::span<const Byte>(profile_vec.data() + exif_end, overflow_size), xmp_data);
			appendSv(xmp_data, XMP_FOOTER);

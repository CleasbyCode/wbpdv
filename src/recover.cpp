...
[[nodiscard]] std::optional<std::size_t> searchSig(std::span<const Byte> vec, const std::array<Byte, N>& sig) {
  if constexpr (N == 0) {
    return 0;
  }
  if (vec.size() < N) {
...
[[nodiscard]] RiffChunkSet findRelevantChunks(std::span<const Byte> riff_file) {
  ...
  for (std::size_t offset = 12; offset < riff_file.size();) {
    ...
    const auto chunk_data = std::span<const Byte>(riff_file.data() + static_cast<std::ptrdiff_t>(data_offset), chunk_size);
...
[[nodiscard]] ByteOrder parseExifByteOrder(std::span<const Byte> exif_data) {
  ...
  if (exif_data[TIFF_HEADER_OFFSET] == 'I' && exif_data[TIFF_HEADER_OFFSET + 1] == 'I') {
...
[[nodiscard]] std::size_t findExifArtistEnd(std::span<const Byte> exif_data) {
  ...
  for (std::uint16_t i = 0; i < ifd_count; ++i) {
    ...
    requireSpanRange(exif_data, artist_data_offset, static_cast<std::size_t>(artist_count), CORRUPT_ERROR);
    return checkedAddSize(artist_data_offset, static_cast<std::size_t>(artist_count), CORRUPT_ERROR);
  }
  throw std::runtime_error(CORRUPT_ERROR);
}
...
void recoverFromIccPath(vBytes& image_vec, std::span<const Byte> iccp_data) {
  ...
  const std::size_t iccp_offset_in_image = static_cast<std::size_t>(iccp_data.data() - image_vec.data());
  const std::size_t offset_within_iccp = *icc_pos - ICC_SIG_OFFSET_FROM_PROFILE_START;
  ...
  std::memmove(image_vec.data(), image_vec.data() + profile_offset, profile_length);
...
void recoverFromBlueskyPath(vBytes& image_vec, std::span<const Byte> exif_data, std::span<const Byte> xmp_data) {
  ...
  vBytes profile(exif_data.begin(), exif_data.begin() + static_cast<std::ptrdiff_t>(artist_end));
  if (!xmp_overflow.empty()) {
    if (profile.size() > MAX_PROGRAM_FILE_SIZE - xmp_overflow.size()) {
      throw std::runtime_error("File Recovery Error: Embedded file exceeds maximum program size.");
    }
    profile.insert(profile.end(), xmp_overflow.begin(), xmp_overflow.end());
  }
...
void recoverData(vBytes& image_vec) {
  ...
  if (chunks.exif.has_value() && hasEmbeddedBlueskyPayload(*chunks.exif)) {
    recoverFromBlueskyPath(image_vec, *chunks.exif, chunks.xmp.value_or(std::span<const Byte>{}));
    return;
  }

...
std::size_t checkedBlueskyProfileSize(std::size_t exif_artist_size,
                                      std::size_t xmp_overflow_size) {
  const std::size_t profile_size = checkedAddSize(
      exif_artist_size, xmp_overflow_size,
      "File Recovery Error: Embedded Bluesky metadata size overflow.");
  if (profile_size > MAX_BLUESKY_UPLOAD_SIZE) {
    throw std::runtime_error(
        "File Recovery Error: Embedded Bluesky metadata exceeds the upload "
        "size limit.");
  }
  return profile_size;
}

fs::path recoveryCollisionCandidate(const fs::path &candidate,
                                    std::size_t index) {
  const std::string filename = candidate.filename().string();
  // Split at the first interior '.' so every part of a multi-part extension
  // stays attached: "archive.tar.gz" -> "archive_1.tar.gz", not the
  // "archive.tar_1.gz" that splitting on the final '.' would produce. The
  // search starts at 1 because a leading '.' is a hidden-file marker rather
  // than a suffix separator (and is rejected upstream anyway).
  const std::size_t suffix_start = filename.find('.', 1);
  std::string_view stem(filename);
  std::string_view extension;
  if (suffix_start != std::string::npos) {
    stem = std::string_view(filename).substr(0, suffix_start);
    extension = std::string_view(filename).substr(suffix_start);
  }
  if (stem.empty()) {
    stem = "recovered";
  }
  return fs::path(std::format("{}_{}{}", stem, index, extension));
}
...
constexpr const char *CORRUPT_EXIF_ERROR =
...
// Offset of the embedded profile within `iccp_data`, or nullopt when this is
// not a wbpdv payload. Returning it saves the recovery path a second scan.
[[nodiscard]] std::optional<std::size_t>
findIccProfileStart(std::span<const Byte> iccp_data) {
  const auto icc_pos = searchSig(iccp_data, ICC_PROFILE_SIGNATURE);
  if (!icc_pos || *icc_pos < ICC_SIG_OFFSET_FROM_PROFILE_START) {
    return std::nullopt;
  }
  const std::size_t profile_start =
      *icc_pos - ICC_SIG_OFFSET_FROM_PROFILE_START;
  const auto profile_data = iccp_data.subspan(profile_start);
  if (!spanHasRange(profile_data, WEBP_EXTRACT_OFFSETS.kdf_metadata,
                    KDF_METADATA_REGION_BYTES)) {
    return std::nullopt;
  }
  // Unlike the Bluesky path, which derives both offsets from where the marker
  // is found, this path decrypts at the fixed template offset. Require the
  // marker exactly where that offset implies rather than anywhere in the
  // profile: a displaced marker can never decrypt, so it should be reported as
  // "not a wbpdv image" instead of costing the user a PIN prompt and a full
  // Argon2id derivation first.
  const auto wdv_pos = searchSig(profile_data, WDV_SIGNATURE);
  if (!wdv_pos || checkedAddSize(*wdv_pos, WDV_SIGNATURE_SPAN,
                                 "Image File Error: Corrupt profile "
                                 "location.") !=
                      WEBP_EXTRACT_OFFSETS.encrypted_file) {
    return std::nullopt;
  }
  return profile_start;
}

// Offset of the WDV marker within `exif_data`, or nullopt when it is absent or
// too close to the start for the KDF metadata that precedes it.
[[nodiscard]] std::optional<std::size_t>
findBlueskyWdvMarker(std::span<const Byte> exif_data) {
  const auto wdv_pos = searchSig(exif_data, WDV_SIGNATURE);
  if (!wdv_pos || *wdv_pos < BLUESKY_WDV_TO_KDF ||
      !spanHasRange(exif_data, *wdv_pos - BLUESKY_WDV_TO_KDF,
                    KDF_METADATA_REGION_BYTES)) {
    return std::nullopt;
  }
  return wdv_pos;
...
[[nodiscard]] ByteOrder parseExifByteOrder(std::span<const Byte> exif_data) {
  requireSpanRange(exif_data, TIFF_HEADER_OFFSET, 8, CORRUPT_EXIF_ERROR);
  ...
  throw std::runtime_error(CORRUPT_EXIF_ERROR);
}

[[nodiscard]] std::size_t findExifArtistEnd(std::span<const Byte> exif_data) {
  requireSpanRange(exif_data, 0, EXIF_HEADER_SIGNATURE.size(),
                   CORRUPT_EXIF_ERROR);
  ...
  const std::size_t ifd_count_offset =
      checkedAddSize(TIFF_HEADER_OFFSET,
                     ...
                     CORRUPT_EXIF_ERROR);
  requireSpanRange(exif_data, ifd_count_offset, 2, CORRUPT_EXIF_ERROR);
  ...
  for (std::uint16_t i = 0; i < ifd_count; ++i) {
    const std::size_t entry_index_bytes =
        checkedMulSize(static_cast<std::size_t>(i), 12, CORRUPT_EXIF_ERROR);
    const std::size_t entry_off =
        checkedSum(CORRUPT_EXIF_ERROR, ifd_count_offset, 2, entry_index_bytes);
    requireSpanRange(exif_data, entry_off, 12, CORRUPT_EXIF_ERROR);
...
constexpr std::size_t MAX_RECOVERY_NAME_ATTEMPTS = 10000;
...
[[nodiscard]] fs::path safeRecoveryPath(std::string decrypted_filename) {
  ...
  if (parsed.has_root_path() || parsed.has_parent_path() ||
      parsed != parsed.filename() || !hasSafeEmbeddedFilename(parsed)) {
    throw std::runtime_error(
        "File Recovery Error: Recovered filename is unsafe.");
  }
...
void writeRecoveredFile(vBytes &image_vec, std::string filename) {
  const fs::path output_base = safeRecoveryPath(std::move(filename));
  auto staged_file = createUniqueFile(
      output_base.parent_path(), ".wbpdv_tmp_", "", 1024,
  ...
  staged_file.closeOrThrow();

  // The atomic no-replace rename is the existence check, so there is no
  // exists-then-rename window: on EEXIST the already-staged bytes are simply
  // retried under the next collision candidate instead of failing.
  fs::path committed_path;
  for (std::size_t i = 0; i <= MAX_RECOVERY_NAME_ATTEMPTS; ++i) {
    const fs::path candidate =
        i == 0 ? output_base : recoveryCollisionCandidate(output_base, i);
    if (tryCommitPathAtomically(
            staged_file.path(), candidate,
  ...
  // Staged path was renamed into place; drop ownership so the destructor does
  // not attempt to unlink the committed destination.
  staged_file.release();
  std::println("\nExtracted hidden file: {} ({} bytes).\n\nComplete! Please "
               "check your file.\n",
               committed_path.string(), image_vec.size());
}
...
void decryptInflateAndWrite(vBytes &image_vec, const ProfileOffsets &offsets,
  ...
  // The recovered payload is plaintext once inflated. Scrub it from the heap on
  // every exit path — including a failed write — matching the wipe policy the
  // rest of the pipeline applies to keys, PINs and compression intermediates.
  SensitiveBytesGuard plaintext_guard(image_vec);
  writeRecoveredFile(image_vec, std::move(*result));
}

void recoverFromIccPath(vBytes &image_vec, std::span<const Byte> iccp_data,
                        std::size_t profile_start) {
  ...
  const std::size_t profile_offset =
      ...
      profile_start;
  const std::size_t profile_length = iccp_data.size() - profile_start;
...
void recoverFromBlueskyPath(vBytes &image_vec, std::span<const Byte> exif_data,
                            ...
                            std::size_t wdv_pos) {
  const std::size_t kdf_offset = wdv_pos - BLUESKY_WDV_TO_KDF;
  const std::size_t encrypted_offset =
      checkedAddSize(wdv_pos, WDV_SIGNATURE_SPAN, CORRUPT_EXIF_ERROR);
  ...
  // is not large enough to hold the full ciphertext. Reject an oversized
  // Artist field before doing any XMP scan or Base64 allocation.
  (void)checkedBlueskyProfileSize(artist_end, 0);
  vBytes xmp_overflow = extractXmpOverflowData(xmp_data);
  const std::size_t profile_size =
      checkedBlueskyProfileSize(artist_end, xmp_overflow.size());

  // The strict Bluesky limit above is checked before allocation, so this copy
  // is bounded to 2,000,000 bytes even if a hostile input contains an enormous
  // EXIF chunk. Moving it into image_vec also releases the original image's
  // potentially much larger backing allocation before PIN derivation/inflate.
  ...
  decryptInflateAndWrite(image_vec, bluesky_offsets, CORRUPT_EXIF_ERROR);
}
} // namespace
...
void recoverData(vBytes &image_vec) {
  ...
  if (chunks.iccp) {
    if (const auto profile_start = findIccProfileStart(*chunks.iccp)) {
      recoverFromIccPath(image_vec, *chunks.iccp, *profile_start);
      return;
    }
  }

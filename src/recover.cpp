...
[[nodiscard]] bool hasEmbeddedIccPayload(std::span<const Byte> iccp_data) {
  ...
  return wdv_pos.has_value() &&
         checkedAddSize(*wdv_pos, WDV_SIGNATURE_SPAN,
                        "Image File Error: Corrupt profile location.") ==
...
[[nodiscard]] bool hasEmbeddedBlueskyPayload(std::span<const Byte> exif_data) {
  ...
  return wdv_pos && *wdv_pos >= BLUESKY_WDV_TO_KDF &&
...
struct PlaintextWipeGuard {
  vBytes &bytes;

  explicit PlaintextWipeGuard(vBytes &in_bytes) : bytes(in_bytes) {}
  PlaintextWipeGuard(const PlaintextWipeGuard &) = delete;
  PlaintextWipeGuard &operator=(const PlaintextWipeGuard &) = delete;
...
void decryptInflateAndWrite(vBytes &image_vec, const ProfileOffsets &offsets,
  ...
  const PlaintextWipeGuard plaintext_guard(image_vec);

...
[[nodiscard]] bool hasEmbeddedIccPayload(std::span<const Byte> iccp_data) {
	...
	if (!icc_pos.has_value() || *icc_pos < ICC_SIG_OFFSET_FROM_PROFILE_START) {
		return false;
	}

	const auto profile_data = iccp_data.subspan(*icc_pos - ICC_SIG_OFFSET_FROM_PROFILE_START);
	if (!spanHasRange(profile_data, WEBP_EXTRACT_OFFSETS.kdf_metadata, KDF_METADATA_REGION_BYTES)) {
		return false;
	}

	return searchSig(profile_data, WDV_SIGNATURE).has_value();
}
...
[[nodiscard]] bool hasEmbeddedBlueskyPayload(std::span<const Byte> exif_data) {
	...
	if (!wdv_pos.has_value() || *wdv_pos < WDV_TO_KDF) {
		return false;
	}

	return spanHasRange(exif_data, *wdv_pos - WDV_TO_KDF, KDF_METADATA_REGION_BYTES);
}
...
void recoverFromIccPath(vBytes& image_vec, std::span<const Byte> iccp_data) {
	...
	auto icc_pos = searchSig(iccp_data, ICC_PROFILE_SIGNATURE);
	...
	if (!spanHasRange(iccp_data, profile_start, 1)) {
		throw std::runtime_error("Image File Error: Corrupt profile location.");
	}

	const auto profile_data = iccp_data.subspan(profile_start);
	image_vec.assign(profile_data.begin(), profile_data.end());

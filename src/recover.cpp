...
[[nodiscard]] std::uint32_t readLe32(const vBytes& v, std::size_t offset) {
...
void recoverFromIccPath(vBytes& image_vec, std::size_t iccp_tag_pos) {
	...
	// Read the ICCP chunk size from 4 bytes after the "ICCP" tag (dynamic, not hardcoded).
	const std::size_t iccp_size_offset = iccp_tag_pos + 4;
	const std::size_t iccp_chunk_size = readLe32(image_vec, iccp_size_offset);
	...
	const std::size_t iccp_data_start = iccp_tag_pos + 8;
...
void recoverFromBlueskyPath(vBytes& image_vec) {
	...
	const std::size_t exif_chunk_size = readLe32(image_vec, size_offset);
...
void recoverData(vBytes& image_vec) {
	...
	if (iccp_tag_pos.has_value() && image_vec.size() >= *iccp_tag_pos + 8) {
		const std::size_t chunk_size = readLe32(image_vec, *iccp_tag_pos + 4);
	...
	if (wdv_in_iccp) {
		...
		recoverFromIccPath(image_vec, *iccp_tag_pos);
	} else {

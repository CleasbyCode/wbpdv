...
[[nodiscard]] std::optional<std::size_t> searchSig(std::span<const Byte> vec, const auto &sig) {
  if (vec.size() < sig.size()) { return std::nullopt; }
...
[[nodiscard]] std::span<const Byte> parseRiffWebP(std::span<const Byte> image_data) {
  ...
  if (!std::ranges::equal(image_data.first(RIFF_CHUNK_TAG.size()), RIFF_CHUNK_TAG) || !std::ranges::equal(image_data.subspan(8, WEBP_CHUNK_TAG.size()), WEBP_CHUNK_TAG)) throw std::runtime_error("Error: Not a valid WEBP image.");
  const std::size_t riff_total_size = 8 + readLe32At(image_data, 4);
  if (riff_total_size - 8 < WEBP_CHUNK_TAG.size() || !spanHasRange(image_data, 0, riff_total_size)) throw std::runtime_error(CORRUPT_RIFF_ERROR);
...
[[nodiscard]] std::optional<RiffChunkView> findChunk(std::span<const Byte> riff_file, const auto &tag) {
  ...
  for (std::size_t offset = 12; offset < riff_file.size();) {
    ...
    const std::size_t data_offset = offset + 8, padded_chunk_size = chunk_size + (chunk_size & 1U);
    if (!spanHasRange(riff_file, data_offset, padded_chunk_size)) throw std::runtime_error(CORRUPT_RIFF_ERROR);
    if (std::ranges::equal(riff_file.subspan(offset, tag.size()), tag)) return RiffChunkView{.data = std::span<const Byte>(riff_file.data() + data_offset, chunk_size)};
...
[[nodiscard]] bool hasEmbeddedIccPayload(std::span<const Byte> iccp_data) {
  ...
  return spanHasRange(profile_data, WEBP_EXTRACT_OFFSETS.kdf_metadata, KDF_METADATA_REGION_BYTES) && searchSig(profile_data, WDV_SIGNATURE).has_value();
}
...
void writeRecoveredFile(vBytes &image_vec, std::string filename) {
  ...
  auto staged_file =
      createUniqueFile(output_path.parent_path(), std::format(".{}.wbpdv_tmp_", output_path.filename().string()), "", 1024,
                       "Write File Error: Unable to create temp output file: ", "Write File Error: Unable to allocate temporary output filename.");
  try {
    ...
    commitPathAtomically(staged_file.path, output_path, "Write File Error: Output file already exists.", "Write File Error: Failed to commit recovered file: ");
  } catch (...) {
...
void recoverFromIccPath(vBytes &image_vec, std::span<const Byte> iccp_data) {
  ...
  requireSpanRange(image_vec, WEBP_EXTRACT_OFFSETS.kdf_metadata, KDF_METADATA_REGION_BYTES, "File Recovery Error: Embedded profile is corrupt.");
...
[[nodiscard]] vBytes extractXmpOverflowData(std::span<const Byte> xmp_data) {
  ...
  if (xmp_data.empty()) { return {}; }
  auto rdfli_pos = searchSig(xmp_data, XMP_RDFLI_SIGNATURE);
  if (!rdfli_pos) { return {}; }
  const auto start_it =
      std::find(xmp_data.begin() + static_cast<std::ptrdiff_t>(*rdfli_pos + XMP_RDFLI_SIGNATURE.size()), xmp_data.end(), static_cast<Byte>('>'));
  if (start_it == xmp_data.end()) { throw std::runtime_error(XMP_ERROR); }
  std::size_t base64_begin = static_cast<std::size_t>(std::distance(xmp_data.begin(), start_it)) + 1;
  while (base64_begin < xmp_data.size() && std::isspace(static_cast<unsigned char>(xmp_data[base64_begin])) != 0) { ++base64_begin; }
  if (base64_begin >= xmp_data.size()) { throw std::runtime_error(XMP_ERROR); }
  auto end_it = std::find(xmp_data.begin() + static_cast<std::ptrdiff_t>(base64_begin), xmp_data.end(), XMP_BASE64_END_DELIM);
  if (end_it == xmp_data.end()) { throw std::runtime_error(XMP_ERROR); }
  std::size_t base64_end = static_cast<std::size_t>(std::distance(xmp_data.begin(), end_it));
  while (base64_end > base64_begin && std::isspace(static_cast<unsigned char>(xmp_data[base64_end - 1])) != 0) { --base64_end; }
  if (base64_end <= base64_begin) { throw std::runtime_error(XMP_ERROR); }
...
void recoverFromBlueskyPath(vBytes &image_vec, std::span<const Byte> exif_data, std::span<const Byte> xmp_data) {
  ...
  if (!wdv_pos.has_value()) { throw std::runtime_error(CORRUPT_ERROR); }
  if (*wdv_pos < WDV_TO_KDF) { throw std::runtime_error(CORRUPT_ERROR); }
  ...
  if (readExif16(exif_data, TIFF_HEADER_OFFSET + 2, byte_order) != 42) { throw std::runtime_error(CORRUPT_ERROR); }
  const std::size_t ifd_count_offset = TIFF_HEADER_OFFSET + static_cast<std::size_t>(readExif32(exif_data, TIFF_HEADER_OFFSET + 4, byte_order));
  ...
  if (ifd_count == 0 || ifd_count > MAX_IFD_ENTRIES) { throw std::runtime_error(CORRUPT_ERROR); }
  ...
  for (std::uint16_t i = 0; i < ifd_count; ++i) {
    ...
    if (tag == 0x013B) {
      ...
      if (type != 0x0002 || artist_count == 0) { throw std::runtime_error(CORRUPT_ERROR); }
      const std::size_t artist_data_offset = TIFF_HEADER_OFFSET + static_cast<std::size_t>(artist_tiff_offset);
      if (!spanHasRange(exif_data, artist_data_offset, static_cast<std::size_t>(artist_count))) { throw std::runtime_error(CORRUPT_ERROR); }
      artist_end = artist_data_offset + static_cast<std::size_t>(artist_count);
      break;
    }
  }
  if (!artist_end.has_value()) { throw std::runtime_error(CORRUPT_ERROR); }
  if (*wdv_pos >= *artist_end || encrypted_offset > *artist_end) { throw std::runtime_error(CORRUPT_ERROR); }

...
constexpr std::size_t MAX_STREAM_SIZE =
...
[[nodiscard]] std::size_t safeFileSize(const fs::path &path) {
  ...
  if (raw_file_size > static_cast<std::uintmax_t>(
...
void syncParentDirectory(const fs::path &file_path) {
  ...
  if (dir_fd.get() < 0) {
    ...
    throw std::runtime_error(std::format(
        "Write Error: Failed to open output directory for sync: {}",
        ec.message()));
  }
  int sync_rc = -1;
  do {
  ...
  if (sync_rc != 0) {
    const std::error_code ec(errno, std::generic_category());
    throw std::runtime_error(std::format(
        "Write Error: Failed to sync output directory: {}", ec.message()));
  }
}
...
renameNoReplace(const fs::path &staged_path, const fs::path &output_path,
  ...
  if (rename_rc == 0) {
    // A directory-sync failure is reported even though the rename has already
    // committed the complete, fdatasync'd file. The final file is deliberately
    // left in place: unlinking it cannot roll back persistence reliably and
    // could destroy the only complete copy.
...
bool hasValidFilename(const fs::path &p) {
  ...
  return std::ranges::all_of(fn, isValidFilenameChar);
}
...
UniqueFileHandle createUniqueFile(const fs::path &directory,
  ...
  for (std::size_t i = 0; i < max_attempts; ++i) {
    const fs::path candidate_name = std::format(
        "{}{}{}", prefix, 100000 + randombytes_uniform(900000), suffix);
    const fs::path candidate =
        directory.empty() ? candidate_name : (directory / candidate_name);
...
void commitPathAtomically(const fs::path &staged_path,
  ...
  if (renameNoReplace(staged_path, output_path, failure_error_prefix) ==
      RenameNoReplaceResult::exists) {

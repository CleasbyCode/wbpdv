...
vBytes readFile(const fs::path &path, FileTypeCheck file_type) {
  ...
  struct ReadWipeGuard {
    vBytes &bytes;
    bool active{true};

    explicit ReadWipeGuard(vBytes &in_bytes) : bytes(in_bytes) {}
    ReadWipeGuard(const ReadWipeGuard &) = delete;
    ReadWipeGuard &operator=(const ReadWipeGuard &) = delete;
...
void commitPathAtomically(const fs::path &staged_path,
                          ...
                          std::string_view failure_error_prefix) {
  if (!tryCommitPathAtomically(staged_path, output_path,
                               failure_error_prefix)) {

...
[[nodiscard]] bool isImageType(FileTypeCheck file_type) { return file_type == FileTypeCheck::cover_image || file_type == FileTypeCheck::embedded_image; }
[[nodiscard]] std::size_t safeFileSize(const fs::path &path) {
  std::error_code ec;
  const std::uintmax_t raw_file_size = fs::file_size(path, ec);
  if (ec) { throw std::runtime_error(std::format("Error: Failed to get file size for \"{}\".", path.string())); }
  if (raw_file_size > static_cast<std::uintmax_t>(std::numeric_limits<std::size_t>::max()) || raw_file_size > static_cast<std::uintmax_t>(MAX_STREAM_SIZE)) {
...
bool hasValidFilename(const fs::path &p) {
  ...
  return !fn.empty() && std::ranges::all_of(fn, [](unsigned char c) { return std::isalnum(c) || c == '.' || c == '-' || c == '_' || c == '@' || c == '%'; });
}
...
std::size_t getFileSizeChecked(const fs::path &path, FileTypeCheck file_type) {
  if (!hasValidFilename(path)) { throw std::runtime_error("Invalid Input Error: Unsupported characters in filename arguments."); }
  std::error_code ec;
  ...
  if (isImageType(file_type)) {
    if (!hasFileExtension(path, {".webp"})) throw std::runtime_error("File Type Error: Invalid image extension. Only expecting \".webp\".");
    if (file_type == FileTypeCheck::cover_image) {
      if (MIN_WEBP_FILE_SIZE > file_size) throw std::runtime_error("File Error: Invalid image file size.");
      if (file_size > MAX_COVER_IMAGE_FILE_SIZE) throw std::runtime_error("Image File Error: Cover image file exceeds maximum size limit.");
    }
  }
  if (file_size > MAX_PROGRAM_FILE_SIZE) throw std::runtime_error("Error: File exceeds program size limit.");
  return file_size;
}
vBytes readFile(const fs::path &path, FileTypeCheck file_type) {
  ...
  if (!file || file.gcount() != expected) { throw std::runtime_error("Failed to read full file: partial read"); }
  return vec;
}
...
void writeAllToFd(int fd, std::span<const Byte> data) {
  ...
  while (written < data.size()) {
    ...
    if (rc == 0) { throw std::runtime_error("Write Error: Failed to write complete output file."); }
...
UniqueFileHandle createUniqueFile(const fs::path &directory, std::string_view prefix, std::string_view suffix, std::size_t max_attempts,
  ...
  for (std::size_t i = 0; i < max_attempts; ++i) {
    ...
    if (fd >= 0) { return UniqueFileHandle{.path = candidate, .fd = fd}; }
...
void commitPathAtomically(const fs::path &staged_path, const fs::path &output_path, std::string_view exists_error, std::string_view failure_error_prefix) {
  ...
  if (errno != ENOSYS && errno != EINVAL) {
    if (errno == EEXIST) { throw std::runtime_error(std::string(exists_error)); }
    const std::error_code ec(errno, std::generic_category());
    throw std::runtime_error(std::format("{}{}", failure_error_prefix, ec.message()));
  }
#endif
  std::error_code ec;
  if (fs::exists(output_path, ec)) { throw std::runtime_error(std::string(exists_error)); }
  if (ec) { throw std::runtime_error(std::format("{}{}", failure_error_prefix, ec.message())); }

...
[[nodiscard]] inline std::uint16_t readLe16At(std::span<const Byte> data, std::size_t offset, std::string_view message = "Internal Error: readLe16At out of bounds.") {
	requireSpanRange(data, offset, 2, message);
	return static_cast<std::uint16_t>(data[offset]) |
		   static_cast<std::uint16_t>(static_cast<std::uint16_t>(data[offset + 1]) << 8);
}

[[nodiscard]] inline std::uint16_t readBe16At(std::span<const Byte> data, std::size_t offset, std::string_view message = "Internal Error: readBe16At out of bounds.") {
	requireSpanRange(data, offset, 2, message);
	return static_cast<std::uint16_t>(static_cast<std::uint16_t>(data[offset]) << 8) |
		   static_cast<std::uint16_t>(data[offset + 1]);
}

[[nodiscard]] inline std::uint32_t readLe32At(std::span<const Byte> data, std::size_t offset, std::string_view message = "Internal Error: readLe32At out of bounds.") {
...
[[nodiscard]] inline std::uint32_t readBe32At(std::span<const Byte> data, std::size_t offset, std::string_view message = "Internal Error: readBe32At out of bounds.") {
	requireSpanRange(data, offset, 4, message);
...
[[nodiscard]] UniqueFileHandle createUniqueFile(const fs::path& directory, std::string_view prefix, std::string_view suffix, std::size_t max_attempts, std::string_view create_error_prefix, std::string_view exhausted_error);

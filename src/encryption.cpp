...
[[nodiscard]] KdfMetadataVersion getKdfMetadataVersion(std::span<const Byte> data, std::size_t base_index) {
	...
	return std::ranges::equal(
		std::span(data.data() + base_index + KDF_MAGIC_OFFSET, KDF_METADATA_MAGIC_V2.size()),
		KDF_METADATA_MAGIC_V2) ? KdfMetadataVersion::v2_secretbox : KdfMetadataVersion::none;
}
...
std::optional<std::string> decryptDataFile(vBytes& profile_vec, const ProfileOffsets& offsets) {
	...
	std::ranges::copy_n(
		profile_vec.begin() + static_cast<std::ptrdiff_t>(offsets.kdf_metadata + KDF_SALT_OFFSET),
		salt.size(),
		salt.begin()
	);
	std::ranges::copy_n(
		profile_vec.begin() + static_cast<std::ptrdiff_t>(offsets.kdf_metadata + KDF_NONCE_OFFSET),
		stream_header.size(),
		stream_header.begin()
	);
	...
	const std::span<const Byte> framed_ciphertext(
		profile_vec.data() + static_cast<std::ptrdiff_t>(offsets.encrypted_file),
		ciphertext_length
	);

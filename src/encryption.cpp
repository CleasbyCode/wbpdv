...
[[nodiscard]] std::size_t computeStreamEncryptedSize(std::size_t plaintext_size) {
  ...
  if (plaintext_size > std::numeric_limits<std::size_t>::max() - header_size ||
      chunks > (std::numeric_limits<std::size_t>::max() - header_size - plaintext_size) / per_chunk_overhead) {
...
// Encrypt prefix||body into output_vec (appended). On exit, body is zeroed and cleared.
// Writing directly to output_vec eliminates a ciphertext-sized memcpy that the caller
// would otherwise do to splice an intermediate ciphertext buffer into its output.
void encryptWithSecretStream(vBytes& output_vec, std::span<const Byte> prefix, vBytes& body, const Key& key, StreamHeader& header) {
  ...
  const std::size_t total = checkedAddSize(prefix.size(), body.size(), "Data File Error: Data file too large to encrypt.");
  output_vec.reserve(checkedAddSize(output_vec.size(), computeStreamEncryptedSize(total), "Data File Error: Encrypted data is too large."));
  output_vec.insert(output_vec.end(), header.begin(), header.end());
  ...
  while (true) {
    ...
    if (chunk_len != 0) {
      if (logical_offset < prefix.size()) {
        ...
        const std::size_t from_prefix = std::min(chunk_len, prefix.size() - logical_offset);
        std::memcpy(staging.data(), prefix.data() + logical_offset, from_prefix);
    ...
    if (crypto_secretstream_xchacha20poly1305_push(
            &state, cipher_chunk.data(), &clen_ull, chunk_ptr, chunk_len, nullptr, 0, tag) != 0) {
    ...
    output_vec.insert(output_vec.end(), cipher_chunk.begin(), cipher_chunk.begin() + static_cast<std::ptrdiff_t>(clen));
...
[[nodiscard]] bool
decryptWithSecretStreamInPlace(vBytes& storage, std::size_t cipher_start, std::size_t cipher_len, const Key& key, const StreamHeader& header) {
  ...
  if (!std::ranges::equal(std::span<const Byte>(storage.data() + cipher_start, header.size()), header)) {
    return false;
  }
  crypto_secretstream_xchacha20poly1305_state state{};
  ...
  while (cipher_pos < cipher_end) {
    ...
    if (frame_len < crypto_secretstream_xchacha20poly1305_ABYTES || frame_len > cipher_end - cipher_pos) {
      return fail();
    }
...
std::uint64_t encryptDataFile(vBytes& profile_vec, vBytes& data_vec, const std::string& data_filename, const ProfileOffsets& offsets) {
  ...
  vBytes prefix_vec(prefix_len);
  prefix_vec[0] = static_cast<Byte>(data_filename.size());
  std::memcpy(prefix_vec.data() + 1, data_filename.data(), data_filename.size());
  ...
  encryptWithSecretStream(profile_vec, std::span<const Byte>(prefix_vec.data(), prefix_vec.size()), data_vec, key.key, stream_header);
...
std::optional<std::string> decryptDataFile(vBytes& profile_vec, const ProfileOffsets& offsets) {
  ...
  if (!decryptWithSecretStreamInPlace(profile_vec, offsets.encrypted_file, ciphertext_length, key.key, stream_header)) {
    return std::nullopt;
  }

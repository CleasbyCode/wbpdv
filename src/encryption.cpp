...
void encryptWithSecretStream(std::span<const Byte> prefix, vBytes& body, const Key& key, StreamHeader& header) {
  ...
  vBytes encrypted;
  encrypted.reserve(computeStreamEncryptedSize(total));
...
std::uint64_t encryptDataFile(vBytes& profile_vec, vBytes& data_vec, const std::string& data_filename, const ProfileOffsets& offsets) {
  ...
  encryptWithSecretStream(std::span<const Byte>(prefix_vec.data(), prefix_vec.size()), data_vec, key.key, stream_header);
  profile_vec.reserve(checkedAddSize(profile_vec.size(), data_vec.size(), "Data File Error: Encrypted data is too large."));

...
struct SensitiveKeyData {
  Key key{};
  ~SensitiveKeyData() { sodium_memzero(key.data(), key.size()); }
...
struct SensitiveBytesGuard {
  ...
  ~SensitiveBytesGuard() {
    if (active && bytes != nullptr && !bytes->empty()) { sodium_memzero(bytes->data(), bytes->size()); }
  }
...
void encryptWithSecretStream(vBytes &plaintext, const Key &key, StreamHeader &header) {
  ...
  while (!emitted_final) {
    ...
    if (crypto_secretstream_xchacha20poly1305_push(&state, cipher_chunk.data(), &clen_ull,
                                                   (chunk_len == 0) ? nullptr : (plaintext.data() + static_cast<std::ptrdiff_t>(offset)), chunk_len, nullptr, 0,
                                                   tag) != 0) {
    ...
    encrypted.insert(encrypted.end(), cipher_chunk.begin(), cipher_chunk.begin() + static_cast<std::ptrdiff_t>(clen));
    offset += chunk_len;
    emitted_final = is_final;
  }
  if (!plaintext.empty()) { sodium_memzero(plaintext.data(), plaintext.size()); }
  plaintext = std::move(encrypted);
}
[[nodiscard]] std::optional<vBytes> decryptWithSecretStream(std::span<const Byte> framed_ciphertext, const Key &key, const StreamHeader &header) {
  if (!spanHasRange(framed_ciphertext, 0, header.size())) { return std::nullopt; }
  if (!std::ranges::equal(framed_ciphertext.first(header.size()), header)) { return std::nullopt; }
  crypto_secretstream_xchacha20poly1305_state state{};
  if (crypto_secretstream_xchacha20poly1305_init_pull(&state, header.data(), key.data()) != 0) { return std::nullopt; }
  vBytes decrypted;
  ...
  while (offset < framed_ciphertext.size()) {
    if (framed_ciphertext.size() - offset < STREAM_FRAME_LEN_BYTES) { return std::nullopt; }
    const std::uint32_t frame_len = readFrameLen(framed_ciphertext, offset);
    offset += STREAM_FRAME_LEN_BYTES;
    if (frame_len < crypto_secretstream_xchacha20poly1305_ABYTES || frame_len > framed_ciphertext.size() - offset) { return std::nullopt; }
    const std::size_t max_plain_chunk = static_cast<std::size_t>(frame_len) - crypto_secretstream_xchacha20poly1305_ABYTES;
    if (max_plain_chunk > plain_chunk.size()) { return std::nullopt; }
    unsigned long long mlen = 0;
    unsigned char tag = 0;
    if (crypto_secretstream_xchacha20poly1305_pull(&state, plain_chunk.data(), &mlen, &tag, framed_ciphertext.data() + static_cast<std::ptrdiff_t>(offset),
                                                   frame_len, nullptr, 0) != 0) {
      return std::nullopt;
    }
    if (mlen > max_plain_chunk) { return std::nullopt; }
    decrypted.insert(decrypted.end(), plain_chunk.begin(), plain_chunk.begin() + static_cast<std::ptrdiff_t>(mlen));
    offset += frame_len;
  ...
  if (!has_final_tag || offset != framed_ciphertext.size()) { return std::nullopt; }
...
void deriveKeyFromPin(Key &out_key, std::uint64_t pin, const Salt &salt) {
  ...
  const int rc = crypto_pwhash(out_key.data(), out_key.size(), pin_buf.data(), pin_len, salt.data(), crypto_pwhash_OPSLIMIT_INTERACTIVE,
                               crypto_pwhash_MEMLIMIT_INTERACTIVE, crypto_pwhash_ALG_ARGON2ID13);
  sodium_memzero(pin_buf.data(), pin_buf.size());
  if (rc != 0) { throw std::runtime_error("KDF Error: Unable to derive encryption key."); }
}
...
[[nodiscard]] std::uint64_t generateRecoveryPin() {
  std::uint64_t pin = 0;
  while (pin == 0) { randombytes_buf(&pin, sizeof(pin)); }
  return pin;
}
[[nodiscard]] std::string extractFilenamePrefix(vBytes &payload) {
  ...
  if (payload.empty() || payload[0] == 0) throw std::runtime_error(CORRUPT_FILE_ERROR);
...
std::uint64_t encryptDataFile(vBytes &profile_vec, vBytes &data_vec, const std::string &data_filename, const ProfileOffsets &offsets) {
  ...
  if (offsets.encrypted_file != profile_vec.size()) { throw std::runtime_error(CORRUPT_PROFILE_ERROR); }
  ...
  if (data_vec.size() > std::numeric_limits<std::size_t>::max() - prefix_len) { throw std::runtime_error("Data File Error: Data file too large to encrypt."); }
  const std::size_t payload_size = data_vec.size();
  data_vec.resize(payload_size + prefix_len);
  if (payload_size > 0) { std::memmove(data_vec.data() + static_cast<std::ptrdiff_t>(prefix_len), data_vec.data(), payload_size); }
...
std::optional<std::string> decryptDataFile(vBytes &profile_vec, const ProfileOffsets &offsets) {
  constexpr const char *CORRUPT_FILE_ERROR = "File Recovery Error: Embedded profile is corrupt.";
  requireSpanRange(profile_vec, offsets.kdf_metadata, KDF_METADATA_REGION_BYTES, CORRUPT_FILE_ERROR);
  if (offsets.encrypted_file > profile_vec.size()) { throw std::runtime_error(CORRUPT_FILE_ERROR); }
  ...
  std::ranges::copy_n(profile_vec.begin() + static_cast<std::ptrdiff_t>(offsets.kdf_metadata + KDF_NONCE_OFFSET), stream_header.size(), stream_header.begin());
  ...
  if (ciphertext_length < min_stream_cipher_size) { throw std::runtime_error(CORRUPT_FILE_ERROR); }
  const std::span<const Byte> framed_ciphertext(profile_vec.data() + static_cast<std::ptrdiff_t>(offsets.encrypted_file), ciphertext_length);

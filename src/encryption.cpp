...
// Outcome of one pass of interactive PIN entry. `value` is 0 whenever the
// input was unusable, which callers treat as a PIN that cannot decrypt.
// `interrupted_by` carries the signal that cut entry short (0 if none) so the
// caller can re-raise it once the terminal has been restored.
struct PinEntry {
  std::uint64_t value{0};
  int interrupted_by{0};
};

[[nodiscard]] PinEntry readPinOnce() {
...
[[nodiscard]] std::uint64_t getPin() {
  while (true) {
    const PinEntry entry = readPinOnce();
    if (entry.interrupted_by == 0) {
      return entry.value;
    }
    // The terminal state and the original signal dispositions have already been
    // restored, so re-raising gives the signal its normal semantics: SIGINT,
    // SIGTERM, SIGHUP and SIGQUIT end the process here and never return.
    (void)::raise(entry.interrupted_by);
    // Reached only when the signal did not end the process: SIGTSTP merely
    // stops it, and raise() returns once the job is resumed (the same is true
    // of a disposition inherited as SIG_IGN). Take the PIN again instead of
    // returning the failure sentinel, which would make a suspend/resume
    // indistinguishable from "Invalid PIN or file is corrupt". Each pass
    // installs a fresh handler and clears the pending signal, so this can only
    // repeat when a genuinely new signal arrives.
  }
}
} // namespace

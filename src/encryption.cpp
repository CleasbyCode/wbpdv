...
// writeKdfMetadata fills every byte of the KDF3 region, so it needs no random
// filler to avoid publishing uninitialized memory; only KDF2 left undefined
// bytes, which is why it randomized the region first. These asserts keep the
// fields contiguous and gap-free so that stays true.

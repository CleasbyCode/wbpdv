...
[[nodiscard]] std::uint32_t readFrameLen(std::span<const Byte> data,
                                         std::size_t index) {
...
void encryptWithSecretStream(vBytes &output_vec, std::span<const Byte> prefix,
  ...
  while (true) {
    ...
    output_vec.insert(output_vec.end(), cipher_chunk.begin(),
...
void writeKdfMetadata(vBytes &profile_vec, const ProfileOffsets &offsets,
  ...
  std::ranges::copy(KDF_METADATA_MAGIC_V2,
                    profile_vec.begin() +
...
void readKdfMetadata(const vBytes &profile_vec, const ProfileOffsets &offsets,
  ...
  std::ranges::copy_n(
      ...
      salt.size(), salt.begin());
  std::ranges::copy_n(
      profile_vec.begin() +
          static_cast<std::ptrdiff_t>(offsets.kdf_metadata + KDF_NONCE_OFFSET),
      stream_header.size(), stream_header.begin());
}

[[nodiscard]] KdfMetadataVersion
getKdfMetadataVersion(std::span<const Byte> data, std::size_t base_index) {
  ...
  return std::ranges::equal(
             std::span(data.data() + base_index + KDF_MAGIC_OFFSET,
...
[[nodiscard]] vBytes makeFilenamePrefix(const std::string &data_filename) {
  ...
  vBytes prefix_vec(1 + data_filename.size());
  prefix_vec[0] = static_cast<Byte>(data_filename.size());
  std::memcpy(prefix_vec.data() + 1, data_filename.data(),
              data_filename.size());
  return prefix_vec;
}
...
std::uint64_t encryptDataFile(vBytes &profile_vec, vBytes &data_vec,
  ...
  vBytes prefix_vec = makeFilenamePrefix(data_filename);
  const std::size_t prefix_len = prefix_vec.size();
  ...
  encryptWithSecretStream(profile_vec, byteSpan(prefix_vec), data_vec, key.key,
                          stream_header);
...
std::optional<std::string> decryptDataFile(vBytes &profile_vec,
  ...
  const KdfMetadataVersion metadata_version =
      getKdfMetadataVersion(profile_vec, offsets.kdf_metadata);

int wdvOut(const std::string& IMAGE_FILENAME) {
	constexpr uint32_t MAX_FILE_SIZE = 10 * 1024U * 1024U; 	// 10MB.
	...
	if (!image_file_ifs || IMAGE_FILE_SIZE > MAX_FILE_SIZE) {
		std::cerr << (!image_file_ifs 
			...
			: "\nImage File Error: Image exceeds the maximum size limit (10MB) for this program.")
		<< ".\n\n";
		return 1;
	}
	...
	if (extract_success_byte_val != 0x90) {
		std::fstream file(IMAGE_FILENAME, std::ios::in | std::ios::out | std::ios::binary);
		std::streampos success_index = WDV_SIG_INDEX + INDEX_DIFF - 1;

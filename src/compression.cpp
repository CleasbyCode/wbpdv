...
void appendOutputChunk(vBytes& result, std::span<const Byte> chunk, std::size_t max_output_size, std::string_view limit_error) {
  if (chunk.empty())
    return;
  if (chunk.size() > max_output_size || result.size() > max_output_size - chunk.size())
    throw std::runtime_error(std::string(limit_error));
  result.insert(result.end(), chunk.begin(), chunk.end());
}

bool refillInput(z_stream& strm, std::span<const Byte> input, std::size_t& input_offset) {
  if (strm.avail_in != 0) {
    return true;
  }
  ...
  const std::size_t chunk_size = std::min<std::size_t>(input.size() - input_offset, std::numeric_limits<uInt>::max());
...
[[nodiscard]] std::span<const Byte> writtenOutput(std::span<const Byte> buffer, const z_stream& strm) {
...
[[nodiscard]] vBytes inflateToVector(std::span<const Byte> input, std::size_t max_output_size) {
  ...
  auto flush_output = [&] {
    appendOutputChunk(result,
                      writtenOutput(std::span<const Byte>(buffer.data(), buffer.size()), strm),
                      max_output_size,
                      "Zlib Compression Error: Inflated data exceeds maximum program size limit.");
...
void zlibDeflate(vBytes& data_vec, std::size_t max_output_size) {
  ...
  auto flush_output = [&] {
    appendOutputChunk(result,
                      writtenOutput(std::span<const Byte>(buffer.data(), buffer.size()), strm),
                      max_output_size,
                      "Zlib Compression Error: Compressed data exceeds maximum output size limit.");
    resetOutput(strm, buffer);
  };
  refillInput(strm, std::span<const Byte>(data_vec.data(), data_vec.size()), input_offset);
  while (true) {
    resetOutput(strm, buffer);
...
void zlibInflate(vBytes& data_vec) {
  VectorWipeGuard input_guard(data_vec);
  vBytes inflated = inflateToVector(data_vec, MAX_INFLATED_OUTPUT_SIZE);

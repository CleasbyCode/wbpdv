...
void appendOutputChunk(vBytes& result, std::span<const Byte> buffer, std::size_t written, std::size_t max_output_size, std::string_view limit_error) {
  if (written == 0)
    return;
...
[[nodiscard]] vBytes inflateToVector(std::span<const Byte> input, std::size_t max_output_size) {
  ...
  auto flush_output = [&] {
    ...
    appendOutputChunk(result,
                      std::span<const Byte>(buffer.data(), written),
                      written,
                      max_output_size,

...
struct ZlibGuard {
  ...
  explicit ZlibGuard(bool use_inflate, int level = Z_BEST_COMPRESSION) : inflate_mode(use_inflate) {
    ...
    if (rc != Z_OK) { throw std::runtime_error(inflate_mode ? "zlib inflateInit failed." : "zlib deflateInit failed."); }
  }
  ~ZlibGuard() { inflate_mode ? inflateEnd(&strm) : deflateEnd(&strm); }
...
[[nodiscard]] vBytes inflateToVector(std::span<const Byte> input, std::size_t max_output_size) {
  ...
  auto refill_input = [&] {
    ...
    const std::size_t chunk_size = std::min<std::size_t>(input.size() - input_offset, static_cast<std::size_t>(std::numeric_limits<uInt>::max()));
...
void zlibDeflate(vBytes &data_vec) {
  ...
  const uLong bound = deflateBound(&strm, static_cast<uLong>(std::min(data_vec.size(), static_cast<std::size_t>(std::numeric_limits<uLong>::max()))));
  if (bound <= std::numeric_limits<std::size_t>::max()) result.reserve(static_cast<std::size_t>(bound));
  std::size_t input_offset = 0;
  auto refill_input = [&] {
    if (strm.avail_in != 0 || input_offset >= data_vec.size()) return;
  ...
  while (true) {
    ...
    if (ret == Z_STREAM_ERROR) throw std::runtime_error("zlib deflate failed.");
    flush_output();
    if (ret == Z_STREAM_END) break;
...
void zlibInflate(vBytes &data_vec) { data_vec = inflateToVector(data_vec, MAX_INFLATED_OUTPUT_SIZE); }

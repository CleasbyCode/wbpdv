...
void zlibDeflate(vBytes &data_vec, std::size_t max_output_size) {
  ...
  // this is a clean drop-in: ~1.6x faster than zlib level 9 at equivalent
  // ratio. Level 9 (not 12 — the "ultra" tier) matches Z_BEST_COMPRESSION.
  if (data_vec.size() <= LIBDEFLATE_WHOLE_BUFFER_LIMIT) {
    LibdeflateCompressorGuard compressor(9);
    if (!compressor.c) {
    ...
    const std::size_t produced = libdeflate_zlib_compress(
        compressor.c, data_vec.data(), data_vec.size(), result.data(), bound);
    if (produced == 0) {

...
[[nodiscard]] bool extractLossyVp8Chunk(std::span<const Byte> webp_data, vBytes &vp8_chunk) {
  if (!spanHasRange(webp_data, 0, WEBP_HEADER_LENGTH)) { return false; }
  if (!std::equal(RIFF_TAG.begin(), RIFF_TAG.end(), webp_data.begin()) || !std::equal(WEBP_TAG.begin(), WEBP_TAG.end(), webp_data.begin() + 8)) {
    return false;
  }
  const std::size_t riff_payload_size = readLe32At(webp_data, 4);
  if (riff_payload_size < WEBP_TAG.size()) { return false; }
  const std::size_t riff_total_size = 8 + riff_payload_size;
  if (!spanHasRange(webp_data, 0, riff_total_size)) { return false; }
  for (std::size_t offset = WEBP_HEADER_LENGTH; offset < riff_total_size;) {
    if (!spanHasRange(webp_data, offset, 8)) { return false; }
    ...
    if (!spanHasRange(webp_data, offset, total_chunk_bytes)) { return false; }
    if (std::equal(VP8_TAG.begin(), VP8_TAG.end(), webp_data.begin() + static_cast<std::ptrdiff_t>(offset))) {
      vp8_chunk.assign(webp_data.begin() + static_cast<std::ptrdiff_t>(offset), webp_data.begin() + static_cast<std::ptrdiff_t>(offset + total_chunk_bytes));
      return true;
    }
...
void reEncodeAsLossy(vBytes &image_vec, float quality) {
  ...
  if (!rgba) throw std::runtime_error("Image File Error: Failed to decode WEBP image for re-encoding.");
  if (w <= 0 || w > std::numeric_limits<int>::max() / 4) { WebPFree(rgba); throw std::runtime_error("Image File Error: Unsafe WEBP width for re-encoding."); }
  uint8_t *output = nullptr;
  const std::size_t output_size = WebPEncodeRGBA(rgba, w, h, w * 4, quality, &output);
  WebPFree(rgba);
  if (output_size == 0 || !output) { if (output) WebPFree(output); throw std::runtime_error("Image File Error: Failed to re-encode WEBP image."); }
  image_vec.assign(output, output + output_size); WebPFree(output);
}
} // namespace
ImageInfo validateAndPrepareImage(vBytes &image_vec) {
  ...
  if (feature_status != VP8_STATUS_OK) { throw std::runtime_error("Error: Not a valid WEBP image."); }
  if (features.width <= 0 || features.height <= 0) { throw std::runtime_error("Image File Error: Invalid WEBP dimensions."); }
  const std::uint64_t pixel_count = static_cast<std::uint64_t>(features.width) * static_cast<std::uint64_t>(features.height);
  if (pixel_count > MAX_COVER_IMAGE_PIXELS) { throw std::runtime_error("Image File Error: Cover image dimensions exceed safe decode limits."); }
  if (features.has_animation != 0) { throw std::runtime_error("Image File Error: WEBP animation image files not supported."); }
  ...
  if (image_vec.size() < WEBP_HEADER_LENGTH) { throw std::runtime_error("Error: Re-encoded WEBP image too small."); }
  if (!std::equal(RIFF_TAG.begin(), RIFF_TAG.end(), image_vec.begin()) || !std::equal(WEBP_TAG.begin(), WEBP_TAG.end(), image_vec.begin() + 8)) {

...
void reEncodeAsLossy(vBytes &image_vec, float quality) {
  int w = 0;
  int h = 0;
  WebPBuffer rgba(WebPDecodeRGBA(image_vec.data(), image_vec.size(), &w, &h));
  if (!rgba) {
    throw std::runtime_error(
        "Image File Error: Failed to decode WEBP image for re-encoding.");
  }
  ...
  const std::size_t output_size =
      WebPEncodeRGBA(rgba.data, w, h, w * 4, quality, output.outParam());
  ...
  image_vec.assign(output.data + loc->first,
                   output.data + loc->first + loc->second);
}
} // namespace
